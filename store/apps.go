...
func (s *Store) CreateApp(ctx context.Context, name string) (app.App, error) {
	const q = `
		...
		VALUES ($1)
		RETURNING id, name, port, deployment_snapshot, created_at, updated_at
	`

	var a app.App
	err := s.pool.QueryRow(ctx, q, name).Scan(&a.ID, &a.Name, &a.Port, &a.DeploymentSnapshot, &a.CreatedAt, &a.UpdatedAt)
	if err != nil {
...
func (s *Store) GetApp(ctx context.Context, id string) (app.App, error) {
	const q = `
		SELECT id, name, port, deployment_snapshot, created_at, updated_at
		FROM apps
		WHERE id = $1
	`

	var a app.App
	err := s.pool.QueryRow(ctx, q, id).Scan(&a.ID, &a.Name, &a.Port, &a.DeploymentSnapshot, &a.CreatedAt, &a.UpdatedAt)
...
func (s *Store) ListApps(ctx context.Context) ([]app.App, error) {
	...
	for rows.Next() {
		var a app.App
		if err := rows.Scan(&a.ID, &a.Name, &a.Port, &a.DeploymentSnapshot, &a.CreatedAt, &a.UpdatedAt); err != nil {
...
func (s *Store) UpdateAppPort(ctx context.Context, id string, port int) (app.App, error) {
	...
	`
	var a app.App
	err := s.pool.QueryRow(ctx, q, id, port).Scan(&a.ID, &a.Name, &a.Port, &a.DeploymentSnapshot, &a.CreatedAt, &a.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return app.App{}, ErrNotFound
	}
	if err != nil {
